...
class DB:
    ...
    # DB MODS
    def addAgent(
        self, ip, timestamp, status
    ):  # INTERNAL, when receive first (setup) ping
        sqlcmd = (
            "insert into agents (agentID, pingtimestamp, status) values (%s, %s, %s)"
        )
    ...
    def dbPull(self):  # PUBLIC
        ...
        self.mycursor.execute(
            "select * from agents order by isnull(service), service asc"
        )
        return self.mycursor.fetchall()

    def pullSpecific(
        self, grouping, value
    ):  # INTERNAL, use this when sending group commands?
        self.checkStatus()
    ...
    def addGrouping(self, ip, typ, grouping):  # PUBLIC
        ...
        for addr in ipArr:
            ...
            print(
                colored(
                    f'[+] Identifier "{grouping}" added to Agent {addr}!\n', "green"
                )
            )
    ...
    def missingStatus(self, ip):  # INTERNAL, after 3 pings missed (timestamp+3min)
        self.mycursor.execute(
            "update agents " "set status = 'MIA'" f"where agentID ='{ip}'"
        )
