...
def start(agentDB):
    ...
    while True:
        ...
        thread = Thread(
            target=handle, args=[data, addr, sock, agentDB, datahold], daemon=True
        )
        thread.start()
...
def handle(data, addr, sock, agentDB, datahold):
    ...
    if (
        "COM" in strdata
    ):  # TODO fix this hsit later, datahold broken and  handling of multiple COMOs (ex. sending command to multiple sources and receiving output?)???
