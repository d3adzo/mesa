...
def mesaPrompt(TS):  # TS is teamserver object
    ...
    MesaCompleter = WordCompleter(baseCMDs, ignore_case=True)
...
def dbPrompt(TS):
    ...
    dbCompleter = WordCompleter(dbCMDs, ignore_case=True)
    while True:
        user_input = (
            prompt(
                "MESA {DB} ~ ",
...
def interactPrompt(interactType, id, TS):
    ...
    interactCompleter = WordCompleter(interactCMDs, ignore_case=True)
    while True:
        user_input = (
            prompt(
                "MESA {" + interactType + "/" + id + "} ~ ",
                history=FileHistory("textfiles/history.txt"),
                auto_suggest=AutoSuggestFromHistory(),
...
def cmdPrompt(TS, interactType, id):
    cmds = ["help", "back"]
    cmdCompleter = WordCompleter(cmds, ignore_case=True)
    while True:
        user_input = (
            prompt(
                "MESA {" + interactType + "/" + id + "/CMD} ~ ",
                history=FileHistory("textfiles/history.txt"),
                auto_suggest=AutoSuggestFromHistory(),
...
def invalid_op():
    print(
        colored('[-] Command not recognized. Enter "help" for command list.\n', "red")
    )
